...
impl Config {
	pub fn init() -> Self {
		Self {
			...
			basic_auth_username: std::env::var("BASIC_AUTH_USERNAME").expect("BASIC_AUTH_USERNAME must be set"),
			basic_auth_password: std::env::var("BASIC_AUTH_PASSWORD").expect("BASIC_AUTH_PASSWORD must be set"),
		}
	}
}
