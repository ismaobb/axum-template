...
pub async fn get_sessions(
	...
	let sessions = Sessions::find()
		...
		.filter(
			Condition::all()
				.add_option(query.created_at.map(|created_at| sessions::Column::CreatedAt.gte(created_at)))
				.add_option(query.user_id.map(|user_id| sessions::Column::UserId.eq(user_id))),
		)
		.all(&app_state.conn)
		.await?
		.into_iter()
