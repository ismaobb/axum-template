...
pub async fn find_users(
	...
	let users = Users::find()
		.filter(
			Condition::all()
				.add_option(query.username.map(|username| users::Column::Username.eq(username)))
...
pub async fn find_user(Path(id): Path<i32>, Extension(state): Extension<Arc<AppState>>) -> Result<ApiOk<Option<Model>>, ApiErr> {
